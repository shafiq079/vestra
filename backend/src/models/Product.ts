import {
  model, models, Schema, type InferSchemaType, type Model, type Types,
} from 'mongoose';
...
export const productVariantSchema = new Schema({
  ...
  colourHex: { type: String, required: true, trim: true },
...
const productSchema = new Schema(
  {
    ...
    genderCollection: { type: String, enum: GENDER_COLLECTIONS, required: true },
    price: { type: Number, required: true, min: 0 },
    salePrice: { type: Number, min: 0 },
    currency: { type: String, required: true, trim: true, uppercase: true, default: 'GBP' },
    images: { type: [productImageSchema], default: [] },
    lifestyleImages: { type: [productImageSchema], default: [] },
    colours: { type: [String], default: [] },
    variants: { type: [productVariantSchema], default: [] },
    availableSizes: { type: [String], default: [] },
    materials: { type: [String], default: [] },
    careInstructions: { type: [String], default: [] },
    fitDescription: { type: String, required: true, trim: true },
    modelInformation: { type: String, trim: true },
    rating: { type: Number, min: 0, max: 5, default: 0 },
    reviewCount: { type: Number, min: 0, default: 0 },
    stockStatus: { type: String, enum: STOCK_STATUSES, default: 'out_of_stock' },
    badges: { type: [{ type: String, enum: PRODUCT_BADGES }], default: [] },
    tryOnEligible: { type: Boolean, default: false },
    sizeRecommendationEligible: { type: Boolean, default: false },
    sizeModelKey: { type: String, trim: true },
...
productSchema.path('salePrice').validate(function validateSalePrice(value?: number) {
  return value === undefined || (this.price !== undefined && value < this.price);
}, 'Sale price must be lower than price');
productSchema.index({ slug: 1 }, { unique: true });
productSchema.index({
  name: 'text', shortDescription: 'text', category: 'text', recommendationTags: 'text',
});
...
export const Product: Model<ProductShape> =
  (models.Product as Model<ProductShape> | undefined) ?? model('Product', productSchema);
