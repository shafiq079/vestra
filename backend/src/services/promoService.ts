...
export interface PromoRule { percentage: number; minimumSpend: number; active: boolean }
export const PROMO_RULES: Readonly<Record<string, PromoRule>> = Object.freeze({
...
export function evaluatePromo(code: string | undefined, subtotal: number): PromoResult {
  const rule = code ? PROMO_RULES[code] : undefined;
